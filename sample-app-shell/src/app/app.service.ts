...
import { IModuleLoadOutput, ModuleLoadService } from '../shared/services/module-load.service';
...
export class AppService {
  ...
  getDynamicModule(moduleName: string): Promise<boolean> {
    ...
    if (this.isCompileModuleAvailable(moduleName)) {
      ...
      dynamicModuleFetchPromise = new Promise<any>((resolve, reject) => {
        self.loadDynamicModule(self.dynamicModulesConfig[moduleName], moduleName)
          .then(() => { resolve(true); })
          .catch(() => { reject(true); });
      });
    }
  ...
  getCompiledComponent(moduleName: string, componentName: string): Promise<ComponentRef<any>> {
    if (!this.compiledModules ||
      !this.compiledModules[moduleName] ||
      !this.compiledModules[moduleName].moduleLoadOutput) {
      return this.loadDynamicModule(this.dynamicModulesConfig[moduleName], moduleName)
        .then(() => {
          return new Promise<ComponentRef<any>>((resolve) => {
            resolve(<any>this.getComponentRef(moduleName, componentName));
          });
        });
    } else {
      return new Promise<ComponentRef<any>>((resolve) => {
  ...
  private getComponentRef(moduleName: string, componentName: string): ComponentRef<any> | undefined {
    ...
    if (moduleRef) {
      const componentFactory = moduleRef
        ...
        .resolveComponentFactory<any>(moduleLoadOutput.moduleAndComponentPkg[componentName]);
      if (componentFactory) {
        cmpRef = componentFactory.create(moduleRef.injector, [], null, moduleRef);
      }
    }
    return cmpRef;
  }

  private loadDynamicModule(modulePath: string, moduleName: string): Promise<void> {
    const self = this;
    return self.moduleLoadService.loadModule(modulePath, moduleName)
