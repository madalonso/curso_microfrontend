import { Compiler, Injectable, ModuleWithComponentFactories, NgModuleFactory } from '@angular/core';
...
export class ModuleLoadService {
  ...
  loadModule(modulePath: string, moduleName: string): Promise<IModuleLoadOutput> {
    return this.loadFromSystemJs(modulePath)
      .then((moduleAndComponentPkg: any) => moduleAndComponentPkg)
      .then((moduleAndComponentPkg: any) => this.compile(moduleAndComponentPkg, moduleName))
  ...
  private compile(moduleAndComponentPkg: any, moduleName: string): Promise<IModuleLoadOutput> {
